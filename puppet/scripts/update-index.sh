...
names+=( "Plex" );         links+=( "http://${_PLEX_LOCAL_DNS}" );         icons+=( "bx-play-circle" );     descriptions+=( "Self-hosted, remotely-accessible, media server" );
names+=( "Sonarr" );       links+=( "http://${_SONARR_LOCAL_DNS}" );       icons+=( "bx-tv" );              descriptions+=( "Smart PVR for newsgroup and bittorrent users" );
names+=( "Radarr" );       links+=( "http://${_RADARR_LOCAL_DNS}" );       icons+=( "bx-movie" );           descriptions+=( "A fork of Sonarr to work with movies" );
names+=( "qBitTorrent" );  links+=( "http://${_QBITTORRENT_LOCAL_DNS}" );  icons+=( "bx-cloud-download" );  descriptions+=( "The qBitTorrent client with an added Web GUI" );
names+=( "Jackett" );      links+=( "http://${_JACKETT_LOCAL_DNS}" );      icons+=( "bx-rss" );             descriptions+=( "API Support for your favorite torrent trackers" );
